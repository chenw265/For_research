...
### 合并其它临床变量
otherclinical=read.csv("LUSCclinical1.csv")
row.names(otherclinical)=otherclinical$bcr_patient_barcode
rt=read.csv("allClinical.csv")
row.names(rt)=rt$Id
sameSample=intersect(row.names(rt), row.names(otherclinical))
otherclinical=otherclinical[sameSample, c("anatomic_neoplasm_subdivision"), drop=F]
other=rt[sameSample,]
rt=cbind(other, otherclinical)
write.csv(rt, file = "allClinical.csv")
